...
7. Repaired the printable bibliography PDF's embedded links: 66 legacy `media.wix.com` link annotations now use the corresponding HTTPS PDFs on `www.leonroth.org`, and the obsolete Littman link now opens the Oxford Academic book page. All 63 distinct migrated PDF destinations returned HTTP 200. The six pages' text, layout, and clickable areas are preserved. The link inventory and verification record are in `migration/bibliography-links.json`; the unavailable Global Dialogue article described below remains unresolved in the PDF.
