...
Create faithful, readable HTML editions while preserving the scans, bibliography citations, and public PDF addresses. The project owner has reviewed and accepted the David Nieto pilot, its review record, and this editorial standard. The archive now contains six complete drafts: David Nieto is accepted, and five works await their own editorial review. At the owner’s request, the latest batch adds three PDFs and simplifies all reading pages by removing inline page markers. All six remain unpublished.
...
Maintain one record per work in `_docs/digitization-catalog.yml`. The seed catalog currently records six transcribed works and one deferred candidate. Expand it by matching the existing bibliographies and `migration/assets.json`; do not rewrite the bibliographies during inventory work. Catalog-only states such as `candidate` or `deferred_source_review` do not create pages and are separate from the four work front-matter statuses.
...
Prioritize short, complete, legible, important works for which a reviewer is available. After the pilots, process batches of three to five works. Keep long books for a later phase, once chapter navigation and the review process have proved themselves on essays. Do not produce large queues of unreviewed OCR.
